(function () {
  ...
  function normalizeRootStack() {
    var targetDocument = findDocument(config.targetDocumentName);
    var rootGroup = findNamedGroup(targetDocument, config.rootGroupName);
    if (rootGroup === null) return "ERROR|ROOT_GROUP_MISSING";
    for (var index = 0; index < config.batchGroupNames.length; index += 1) {
      var batchGroup = findNamedGroup(rootGroup, config.batchGroupNames[index]);
      if (batchGroup !== null && batchGroup.parent === rootGroup) {
        batchGroup.zOrder(ZOrderMethod.BRINGTOFRONT);
      }
    }
    app.redraw();
    return "OK|existing_artwork_preserved=true";
  }

  function assertCompleteArtwork() {
    var targetDocument = findDocument(config.targetDocumentName);
    var rootGroup = findNamedGroup(targetDocument, config.rootGroupName);
    if (rootGroup === null) return "ERROR|ROOT_GROUP_MISSING";
    var missing = 0;
    for (var batchIndex = 0; batchIndex < config.batches.length; batchIndex += 1) {
      var expectedBatch = config.batches[batchIndex];
      var batchGroup = findNamedGroup(rootGroup, expectedBatch.groupName);
      if (batchGroup === null) {
        missing += expectedBatch.paintNames.length;
        continue;
      }
      for (var itemIndex = 0; itemIndex < expectedBatch.paintNames.length; itemIndex += 1) {
        if (findNamedArtwork(batchGroup, expectedBatch.paintNames[itemIndex]) === null) missing += 1;
      }
    }
    return "OK|missing=" + missing + "|placed=" + rootGroup.placedItems.length + "|raster=" + rootGroup.rasterItems.length;
  }
  ...
  try {
    ...
    if (config.operation === "normalize") return normalizeRootStack();
    if (config.operation === "qa") return assertCompleteArtwork();
