...
def parse_atom(element: ET.Element, transform: Transform, presentation: dict[str, str], opacity: float, index: int) -> dict | None:
    ...
    # Some tracing engines emit harmless move-only or zero-length paths.
    # They have paint attributes but no drawable geometry, so omit them from
    # Illustrator playback instead of failing the entire figure.
    if not subpaths:
        return None
